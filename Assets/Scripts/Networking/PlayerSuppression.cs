...
public class PlayerSuppression : MonoBehaviour
{
    ...
    [Range(0f, 1f)]
    public float maxRingVolume = 0.175f;

    [Tooltip("Seconds for the ring to fall away to silence when incoming fire stops.")]
    public float ringFadeOutSeconds = 2.5f;

    [Tooltip("Seconds for the ring to fade to silence after a headshot kill, overriding the normal fade out above.")]
    public float ringHeadshotDeathFadeSeconds = 1.5f;

    [Tooltip("Seconds for the ring to fade to silence after an ordinary (non-headshot) death, overriding the normal fade out above.")]
    public float ringDeathFadeSeconds = 3f;
    ...
    private bool ringDeathFadeActive;
    private float ringDeathFadeDuration;
    ...
    private void Update()
    {
        ...
        if (dead && !wasDead)
        {
            ...
            // Same headshot-vs-ordinary split as the screen fade above,
            // just driving the ring's volume out to silence instead.
            ringDeathFadeActive = true;
            ringDeathFadeDuration = headshotDeath ? ringHeadshotDeathFadeSeconds : ringDeathFadeSeconds;
        }
    ...
    {
        // Death overrides the normal envelope entirely: the ring rides out
        // whatever it was doing at the moment of death, then falls silent
        // over a fixed duration instead of tracking suppression any further.
        float target = ringDeathFadeActive ? 0f : maxRingVolume * Mathf.Max(level, blastShake);
        ...
        // No fade in: the ring should land at full strength with the fire,
        // not swell up to it. Falling is the only part that eases, so it
        // tails off smoothly instead of cutting — death uses its own fixed
        // duration for that, split by whether it was a headshot.
        if (target > ringSource.volume)
        {
            ringSource.volume = target;
            return;
        }

        float seconds = ringDeathFadeActive ? ringDeathFadeDuration : ringFadeOutSeconds;
        float rate = maxRingVolume / Mathf.Max(0.01f, seconds);

        ringSource.volume = Mathf.MoveTowards(ringSource.volume, target, rate * Time.deltaTime);
    }
